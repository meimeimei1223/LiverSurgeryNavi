// FullSphereCamera.h
// 球面カメラシステム（AR内部パラメータ対応 + TargetMode分割画面対応）
...
public:
    ...
    double ZOOM_SENSITIVITY = -1.0;         // ズーム感度
    float SCALE_SPEED = 1.1f;               // スケール変更速度（カッター等）
    ...
    // ターゲットモード（分割画面対応）
    // ========================================
    TargetMode currentTarget = TARGET_CENTER;
    glm::vec3 targetPositions[TARGET_COUNT];
    ...
    // キャリブレーション解像度（OBJ生成時の解像度）
    int calibWidth  = 0;    // 0 = 未設定
    int calibHeight = 0;
    ...
    void Reset() {
        rotation = glm::quat(1.0f, 0.0f, 0.0f, 0.0f);
        gRadius = InitialRadius;  // auto-scaled by applySceneScaleToCamera()
    ...
    // ターゲットを順番に切り替え
    void switchTarget() {
        currentTarget = static_cast<TargetMode>((currentTarget + 1) % TARGET_COUNT);
        ...
        case TARGET_CENTER:
            ...
            break;
        case TARGET_LIVER:
            std::cout << "Liver Mesh" << std::endl;
            break;
        case TARGET_TEXTURE:
            std::cout << "Texture Board" << std::endl;
            break;
        default:
            break;
        }
    ...
    // ターゲット位置を更新（メッシュ中心座標を渡す）
    void updateTargetPositions(const glm::vec3& liverCenter, const glm::vec3& textureCenter) {
        targetPositions[TARGET_CENTER]  = glm::vec3(0.0f, 0.0f, 0.0f);
        targetPositions[TARGET_LIVER]   = liverCenter;
        targetPositions[TARGET_TEXTURE] = textureCenter;
    }
    ...
    //
    // Convention: cx, cy are measured from the image's TOP-LEFT corner
    // (OpenCV / Kinect convention). This matches the format emitted by
    // most calibration tools (opencv, k4a, azure kinect) and by depth-
    // derived OBJ generators (e.g. DepthAnything + pinhole unprojection).
    //
    // The principal-point Y term is negated so that a 3D point on the
    // optical axis (view-space (0, 0, -Z)) lands at screen pixel
    // (cx, cy)-from-top. Earlier versions of this function used the
    // OpenGL "cy-from-bottom" convention, which looks identical for a
    // symmetric principal point (cy = height/2) but drifts by
    // (2*cy - height) pixels vertically otherwise.
    glm::mat4 createProjectionFromIntrinsics() const {
        // キャリブレーション解像度が設定されている場合はそれを使用
        // （ウィンドウリサイズに影響されない正確なprojection）
        int width  = (calibWidth  > 0) ? calibWidth  : getWindowWidth();
        int height = (calibHeight > 0) ? calibHeight : getWindowHeight();
        ...
        proj[2][1] = 2.0f * cy / height - 1.0f;   // OpenCV cy-from-top
    ...
    void Pan(float dx, float dy) {
        ...
        glm::vec3 movement = moveRight + moveUp;
    ...
    void UpdateCamera(float deltaTime = 0.016f) {
        // ターゲットモードによるカメラ注視点の更新（スムーズ遷移対応）
        glm::vec3 desiredTarget = targetPositions[currentTarget];
        if (smoothTransition) {
            float t = transitionSpeed * deltaTime;
            t = glm::clamp(t, 0.0f, 1.0f);
            cameraTarget = glm::mix(cameraTarget, desiredTarget, t);
        } else {
    ...
    void onWindowResize(int width, int height) {
        // キャリブレーション解像度が設定されている場合はcx/cyを保持
        // （キャリブレーション値はウィンドウサイズに依存しない）
        if (useIntrinsics && calibWidth > 0 && calibHeight > 0) {
            // cx/cy はキャリブレーション時の値を維持
            return;
        }
        // キャリブレーション未設定時は画像中心に更新
        if (useIntrinsics) {
            cx = width / 2.0f;
            cy = height / 2.0f;
        }
    ...
    void printCameraInfo() const {
        ...
        std::cout << "Target Mode: " << currentTarget << std::endl;
